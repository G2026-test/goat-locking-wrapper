...
contract Deploy is Script {
    function run() public {
        ...
        address owner = _resolveOwner(deployer);

        console.log("Deployer:", deployer);
        console.log("Owner:", owner);

        vm.startBroadcast(deployerPrivateKey);
        _deployValidatorEntryUpgradeable(owner);
        vm.stopBroadcast();
    }

    function _resolveOwner(address deployer) internal view returns (address) {
        address ownerAddr = vm.envOr("OWNER_ADDR", address(0));
        return ownerAddr == address(0) ? deployer : ownerAddr;
    }

    function _deployValidatorEntryUpgradeable(address owner) internal {
        ...
        bytes memory initData = abi.encodeCall(
            ...
            (ILocking(lockingAddr), IERC20(rewardTokenAddr), foundationAddr, owner)
        );

        ERC1967Proxy proxy = new ERC1967Proxy(address(implementation), initData);
        ValidatorEntryUpgradeable validatorEntry = ValidatorEntryUpgradeable(payable(address(proxy)));
        ...
        console.log("ValidatorEntryUpgradeable implementation:", address(implementation));
        console.log("ValidatorEntryUpgradeable proxy:", address(validatorEntry));
